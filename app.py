...
if features_path.exists():
    ...
    for name, m in models_to_train.items():
        ...
        comparison_models[name] = {"model": m, "train_accuracy": round(train_acc * 100, 1)}
...
def generate_mfcc_image(y, sr):
    ...
    img = librosa.display.specshow(mfcc, sr=sr, hop_length=512, x_axis='time', ax=ax, cmap='coolwarm')
...
def generate_chroma_image(y, sr):
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 3), gridspec_kw={'width_ratios': [3, 2]})

    # Chroma heatmap over time
    img = librosa.display.specshow(chroma, sr=sr, hop_length=512, x_axis='time',
                                    y_axis='chroma', ax=ax1, cmap='magma')
...
def generate_model_comparison_image(model_results):
    ...
    genre_colors = {
        'blues': '#2196F3', 'classical': '#9C27B0', 'country': '#FF9800',
        'disco': '#E91E63', 'hiphop': '#00BCD4', 'jazz': '#FFC107',
        'metal': '#F44336', 'pop': '#4CAF50', 'reggae': '#8BC34A', 'rock': '#FF5722'
    }
    ...
    for bar, pred, conf in zip(bars, predictions, confidences):
        ax.text(bar.get_width() + 1, bar.get_y() + bar.get_height() / 2,
                f'{pred} ({conf}%)', va='center', color='#ccc', fontsize=8.5)
...
def generate_pca_image(X_pca, genres, new_pca, new_genre):
    ...
    for g in np.unique(genres):
        ...
        ax.scatter(X_pca[mask, 0], X_pca[mask, 1], c=color, label=g,
                   alpha=0.45, s=22, edgecolors='none')
    ax.scatter(new_pca[0, 0], new_pca[0, 1], c='#fff', marker='*', s=350,
               edgecolors='#ffe000', linewidths=1.5, zorder=10,
    ...
    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            ax.text(j, i, format(cm[i, j], 'd'), ha='center', va='center',
                    color='white' if cm[i, j] > thresh else '#aaa', fontsize=7)
...
if features_path.exists() and comparison_models:
    ...
    rf_importances = sorted(
        zip(feature_cols, comparison_models["Random Forest"]["model"].feature_importances_),
    ...
    confusion_img_static = generate_confusion_matrix_image(conf_matrix, label_encoder.classes_)
...
def predict():
    ...
    try:
        ...
        comparison_img = generate_model_comparison_image(model_results) if model_results else None
